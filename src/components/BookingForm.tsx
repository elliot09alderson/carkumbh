...
import { Sparkles, CreditCard, Download, QrCode } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { createBooking } from "@/api/bookings";
import { createOrder, verifyPayment } from "@/api/payments";
import { getEventPackages, EventPackage, getEventContent } from "@/api/siteConfig";
...
const BookingForm = () => {
  ...
  useEffect(() => {
    const loadPackages = async () => {
      ...
      try {
        ...
        setRazorpayName(ec.razorpayName || "Toran Sir");
      } catch (_) {}
    };
    loadPackages();
  }, []);
  ...
  const handleCashBooking = async () => {
    ...
    setIsLoading(true);
    try {
      const booking = await createBooking({ ...formData, screenshot: null });
      setToken(booking.token);
      toast({ title: "Booking Confirmed!", description: `Your token: ${booking.token}` });
    } catch (error: any) {
  ...
  const handleOnlinePayment = async () => {
    if (!validateForm()) return;
    if (!razorpayLoaded) {
      toast({ title: "Error", description: "Payment gateway is loading. Please try again.", variant: "destructive" });
      return;
    }
    setIsLoading(true);
    try {
      const orderData = await createOrder({ name: formData.name, number: formData.number, address: formData.address, package: formData.package });
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            toast({ title: "Payment Successful!", description: `Your token: ${verifyData.token}` });
          } catch (error: any) {
        ...
        prefill: { name: formData.name, contact: formData.number },
        theme: { color: "#f97316" },
      ...
      toast({ title: "Error", description: error.response?.data?.message || "Failed to initiate payment.", variant: "destructive" });
      setIsLoading(false);
    }
  };
      ...
      <Card className="p-8 bg-gradient-card border-border/50 backdrop-blur-sm shadow-elevated">
        ...
        {token ? (
          ...
          >
            ...
            <p className="text-muted-foreground mb-4 text-sm">Your event ticket with QR code is ready</p>

            {/* Ticket Card */}
            <div className="bg-card border-2 border-primary/30 rounded-2xl overflow-hidden mb-6 text-left shadow-lg shadow-primary/10">
              {/* Header */}
              <div className="bg-gradient-to-r from-primary to-orange-600 px-6 py-4">
                <p className="text-white/90 text-xs uppercase tracking-widest font-bold">Event Ticket</p>
              </div>

              {/* Dashed divider */}
              <div className="border-t-2 border-dashed border-border mx-4 my-0" />
              ...
              <div className="px-6 py-5 flex flex-col sm:flex-row gap-6 items-center">
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <p className="text-xs font-bold text-primary uppercase tracking-wider">Token</p>
                    <p className="text-3xl font-black font-mono tracking-widest text-primary">{token}</p>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-4">
                    <div>
                      ...
                      <p className="font-bold text-primary">₹{formData.package}</p>
                    </div>
                    <div>
              ...
              <div className="bg-muted/30 px-6 py-2 text-center">
                <p className="text-xs text-muted-foreground">Valid for one-time entry only — non-transferable</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              ...
              {(() => {
                ...
                if (selectedPkg?.whatsappLink) {
                  return (
                    <a href={selectedPkg.whatsappLink} target="_blank" rel="noopener noreferrer" className="flex-1">
            ...
            <Button
              ...
              onClick={() => {
                setToken("");
                setQrDataUrl("");
                setFormData({ name: "", number: "", address: "", package: packages.length > 0 ? packages[0].price : "500", paymentMode: "online" });
              }}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
              <Label htmlFor="name">Full Name</Label>
              <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" placeholder="Enter your name" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.3 }}>
              <Label htmlFor="number">Phone Number</Label>
              <Input id="number" type="tel" value={formData.number} onChange={(e) => { const v = e.target.value.replace(/\D/g, ""); if (v.length <= 10) setFormData({ ...formData, number: v }); }} className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" placeholder="Enter 10 digit phone number" maxLength={10} />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.4 }}>
              <Label htmlFor="address">Address</Label>
              <Input id="address" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" placeholder="Enter your address" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.5 }}>
              <Label className="mb-3 block">Select Package</Label>
              <RadioGroup value={formData.package} onValueChange={(value) => setFormData({ ...formData, package: value })} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {packages.map((pkg) => (
                  <Label key={pkg.id} htmlFor={`package-${pkg.price}`} className={`flex items-center justify-center p-4 rounded-lg border-2 cursor-pointer transition-all ${formData.package === pkg.price ? "border-primary bg-primary/10 shadow-glow" : "border-border/50 bg-secondary/30 hover:border-border"}`}>
                    <RadioGroupItem value={pkg.price} id={`package-${pkg.price}`} className="sr-only" />
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.7 }}>
              <Button type="submit" disabled={isLoading} className="w-full bg-gradient-primary hover:opacity-90 text-primary-foreground font-semibold py-6 text-lg shadow-glow">
                {isLoading ? "Processing..." : formData.paymentMode === "online" ? `Pay ₹${totalAmount}` : "Confirm Booking"}
              </Button>
            </motion.div>
          </form>
        )}
